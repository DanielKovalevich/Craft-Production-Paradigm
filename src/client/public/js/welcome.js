...
function sendToServer(postData) {
  $.ajax({
    ...
    timeout: 5000,
    url: 'https://psu-research-api.herokuapp.com/startGame', 
...
function checkIfPinIsValid() {
  ...
  $.ajax({
    type: 'GET',
    cache: false,
    url: 'https://psu-research-api.herokuapp.com/startGame/checkIfPinExists/' + pin,
    success: (result) => {
      if (result) {
        $('.invalid-pin').addClass('hidden');
...
function getPossiblePositions(pin) {
  $.ajax({
    type: 'GET',
    cache: false,
    url: 'https://psu-research-api.herokuapp.com/startGame/getPossiblePositions/' + pin,
    success: (result) => {
      if (result.length == 0) {
        $('.invalid-pin').removeClass('hidden');
...
function joinGame(pin, position) {
  ...
  $.ajax({
    type: 'POST',
    data: postData,
    url: 'https://psu-research-api.herokuapp.com/startGame/joinGame/' + pin,
    timeout: 5000,
    success: () => {
      sessionStorage.position = position;
...
function updateActivePlayers(pin) {
  $.ajax({
    type: 'GET',
    timeout: 5000,
    url: 'https://psu-research-api.herokuapp.com/startGame/addActivePlayer/' + pin,
